...
namespace EasyCharacterMovement
{
    ...
    public class Character : MonoBehaviour
    {
        ...
        public virtual void TeleportPosition(Vector3 newPosition, bool interpolating = true, bool updateGround = false)
        {
            if (interpolating)
            {
        ...
        public virtual void TeleportRotation(Quaternion newRotation, bool interpolating = true)
        {
            if (interpolating)
            {
                characterMovement.interpolation = RigidbodyInterpolation.None;
        ...
        {
            if (useRootMotion && rootMotionController)
                return rootMotionController.GetRootMotionVelocity();
        ...
        public virtual void Simulate(float deltaTime)
        {
            ...
            // If not using root motion, flush rootmotion accumulated deltas.
            // This prevents accumulation while character is toggling root motion.

            if (rootMotionController && !useRootMotion)
