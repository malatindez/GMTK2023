    ...
    public sealed class CharacterMovement : MonoBehaviour
    {
        ...
        public void AddExplosionForce(float forceMagnitude, Vector3 origin, float explosionRadius, ForceMode forceMode = ForceMode.Force)
        {
           // Do nothing if outside radius
            ...
            float falloff = Mathf.InverseLerp(0.0f, explosionRadius, delta.magnitude);
            float explosionForce = Mathf.Lerp(forceMagnitude, 0.0f, falloff);

            if (explosionForce > 0.0f)
                AddForce(delta.normalized * explosionForce, forceMode);
        }
